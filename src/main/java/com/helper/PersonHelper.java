...
public class PersonHelper extends BasePage {
	...
	public PersonHelper(String id, String title, String author) {
		this.id = id;
		this.title = title;
		this.author = author;
	...
	public void allPersons()
	{
		...
		try {
			response = given().spec(getSingle(EndPoints.SINGLE_PERSON, 8)).when().get();
			ExtentReportConf.reportInfoLog(response.asString());
			valResponse = response.then().spec(getResponse().body("title[0]", equalTo("json-server")));
			ExtentReportConf.reportInfoLog(response.asString());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			ExtentReportConf.reportFailLogwithSS("Error occured while executing and details are "+response.asString(), author);
		}
	...
	public void test4()
	{
		...
		map.put("Author", "Aisha");
		map.put("Id", "4");


		response = given().spec(getQueryParam(EndPoints.ALL_PERSON, map)).when().get();
		ExtentReportConf.reportInfoLog(response.asString());
		valResponse = response.then().spec(getResponse());
		return valResponse;
	...
	public ValidatableResponse test5()
	{
		person = new Person();
		...
		ValidatableResponse	response = given().spec(Post(EndPoints.CREATE_PERSON, person)).when().post();
		ValidatableResponse valResponse = response.then().spec(postResponse());
		ExtentReportConf.reportInfoLog(response.asString());
	...
	public ValidatableResponse put()
	{
		...
		Person.setId(30);
		Person.setTitle("Pop");
		Person.setAuthor("Pops");
		
		ValidatableResponse response = given().spec(Post(EndPoints.UPDATE_PERSON, person,29)).when().put();
		ValidatableResponse valResponse = response.then().spec(updateResponse());
		ExtentReportConf.reportInfoLog(response.asString());
	...
	public ValidatableResponse patch()
	{
		...
		Person.setAuthor("Homes");
		
		ValidatableResponse response = given().spec(update(EndPoints.UPDATE_PERSON, person,29)).when().put();
		ValidatableResponse valResponse = response.then().spec(updateResponse());
		ExtentReportConf.reportInfoLog(response.asString());
