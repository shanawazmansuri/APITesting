...
public class MySelfHelper extends BasePage {
	...
	public ValidatableResponse postTest() {
		...
		ts.setSourceCode("Bamboo");
		ts.setDefectTool("Bugzilla");
		ts.setProgammingLanguage("C#");

		ms.setId(5);
		ms.setTitle("Sweet Family");
		ms.setAuthor("All Family");
		ms.setPrimarySkills(al);
		ms.setTechnical(al);
	...
	public ValidatableResponse putTest() {
		...
		ms.setId(2);
		ms.setTitle("Smart Family");
		ms.setAuthor("Mansuri");
		ms.setPrimarySkills(al);
		ms.setTechnical(al);
	...
	public ValidatableResponse params() {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("Author", "Mansuri");
	...
	public ValidatableResponse path() {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("id", 2);

		response = given().spec(getPathparam(EndPoints.ALL_PERSON, map)).when().get();
		ExtentReportConf.reportInfoLog(response.asString());
		valResponse = response.then().spec(getResponse());
		return valResponse;
